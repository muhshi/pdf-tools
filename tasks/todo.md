...
- [x] **Task 1**: Dapatkan Client ID, Client Secret, & Issuer URI dari Admin SSO Sipetra
- [x] **Task 2**: Update `.env` & `.env.example` dengan konfigurasi OAuth2 Sipetra & `SYSTEM_DEFAULTTHEME=light`

## Checkpoint 1: Configuration Readiness
- [x] File `.env` terkonfigurasi dengan variabel SSO Sipetra & Light Mode

## Phase 2: Theme Customization (Light Mode)
- [x] **Task 3**: Atur `system.defaultTheme: light` pada `extraConfigs/custom_settings.yml` & siapkan `customFiles/static/custom.css`
